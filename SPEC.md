...
## Knocked-out flags on the leaderboard (v3.6)
- The standalone leaderboard (leaderboard.html, built by the Action) now draws a red cross (✕) over the flag of any sweepstake team that's out of the tournament. "Out" is computed from data we already hold: a team is crossed if it lost a completed knockout tie (incl. on penalties) or — once the bracket is fully drawn (32 teams present in the R32 fixtures) — if it failed to reach the Round of 32. Until the groups finish, only confirmed knockout losers are crossed (no false positives).
- Implementation: `fl()` now wraps each flag in its own `<span class="flag">` (adding `out` when eliminated) instead of concatenating bare emoji; a small CSS block overlays a red ✕ (`\2715`) with a white halo via `.flag.out::after`. The knocked-out set is derived in `main()` from the resolved `knockouts.json` fixtures + their scores and passed into `build_html`.
- v3.6 — leaderboard crosses out eliminated teams' flags (KO losers + group-stage non-qualifiers), fully automatic from the bracket/results.
